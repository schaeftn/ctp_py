...
sys.path.insert(0,"/home/tristan/projects/cell_decomposition_py/src")

# noinspection PyUnresolvedReferences
from org.combinators.ctp.py.celldecomposition.scene_object import SceneObjectBox3D

# export PYTHONPATH=/home/tristan/projects/cell_decomposition_py:$PYTHONPATH
...
def test_route(points, min_distance):
    ...
    print(f"asd: {r_array}")
    point_array = get_points_triangular(ch.points[ch.vertices[0]], ch.points[r_array[0]], ch.points[r_array[1]], min_distance)
    ...
    # calc N from min_distance, triangle
    # add y element
...
# def run_path_planning(start, end):
# TODO find vectors
# def find_plane_vectors(v1,v2,slice_orth)
def sample_angles(initial_alpha, initial_beta):
    print(f"sample_angles: ")
    # sample x,y gaussian in 2D -> chose optimal path
...
def run_slice_test():
    ...
    print("now:::")
    path_array = test_route_mult(mesh2, 0.5)
    print(f"patharray: {path_array}")
    functools.reduce(lambda a, b: a + b, path_array)

    #TODO foreach test run sample... path refine
